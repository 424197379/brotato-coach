...
func _brotato_coach_install_button():
	var new_run_button = _brotato_coach_new_run_button()
	if not is_instance_valid(new_run_button):
		return
	var parent = new_run_button.get_parent()
	if parent == null:
		return
	if parent.get_node_or_null("BrotatoCoachRunEndButton") != null:
		return
	var following = _brotato_coach_next_focusable_sibling(parent, new_run_button)
	...
	parent.add_child_below_node(new_run_button, _brotato_coach_button)
	_brotato_coach_link_horizontal_focus(new_run_button, _brotato_coach_button, following)
...
func _brotato_coach_on_pressed():
	...
	coordinator.analyze_and_show(self, snapshot, "run_end", _brotato_coach_button)


func _brotato_coach_new_run_button():
